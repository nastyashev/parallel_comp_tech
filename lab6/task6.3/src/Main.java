...
class Main {
    ...
    private static final int NRA = 2048; // рядки матриці А
    private static final int NCA = 2048; // стовпці матриці А
    private static final int NRB = NCA; // рядки матриці В
    private static final int NCB = 2048; // стовпці матриці В
    private static final int NRC = NRA; // рядки матриці С
    private static final int NCC = NCB; // стовпці матриці С
    private static final int N_SEND = 2048; // кількість елементів, які можна відправити за один раз
...
//                    b[i][j] = Math.random() * 10;
    ...
    private static void sendMatrix(double[][] matrix, int dest, int from) {
        ...
        for (int i = 0; i < nCountSend; i++) {
            ...
            MPI.COMM_WORLD.Isend(tmp, i * N_SEND, nSend, MPI.DOUBLE, dest, from); // відправлення блоку
        }
    }
    ...
    private static double[][] receiveMatrix(int source, int rows, int cols, int from) {
        ...
        Request[] r = new Request[nCountRecv]; // запити на отримання блоків
        // Отримання блоків
        for (int i = 0; i < nCountRecv; i++) {
            int nRecv = Math.min(N_SEND, rows * cols - i * N_SEND); // кількість елементів в поточному блоку
            r[i] = MPI.COMM_WORLD.Irecv(tmp, i * N_SEND, nRecv, MPI.DOUBLE, source, from); // отримання блоку
        }
    ...
    private static void sendInt(int value, int dest, int from) {
        ...
        MPI.COMM_WORLD.Isend(tmp, 0, 1, MPI.INT, dest, from); // відправлення цілого числа
    }
    ...
    private static int receiveInt(int source, int from) {
        int[] tmp = new int[1]; // тимчасовий масив для зберігання цілого числа
        Request r = MPI.COMM_WORLD.Irecv(tmp, 0, 1, MPI.INT, source, from); // отримання цілого числа
        r.Wait(); // очікування завершення отримання
